"""
자동 미분을 통해 신경망의 역전파 단계 연산을 자동화할 수 있다.
pytorch 의 autograd 패키지가 이 자동 미분 기능을 제공한다.
autograd는 순전파 단계에서 연산 그래프를 정의하게 된다. (노드:텐서, 엣지:출력텐서를 만드는 함수)

x 가 x.requires_grad=True인 텐서일 때, x.grad 는 x의 변화도를 갖는 또 다른 텐서이다.
"""
